init python in snake:
    from store import Color, persistent
    ...
    class Snake(renpy.Displayable):
        ...
        def reset(self):
            if self.score > persistent.hiscore:
                persistent.hiscore = self.score
            else:
                persistent.hiscore = persistent.hiscore
            self.score = 0
    ...
    class SnakeOverlay(renpy.Displayable):
        ...
        def get_high_score(self):
            return persistent.hiscore

        def imagebuttoneventup(self):
            self.imagebuttonevent(buttondirection="up")
        def imagebuttoneventright(self):
            self.imagebuttonevent(buttondirection="right")
        def imagebuttoneventdown(self):
            self.imagebuttonevent(buttondirection="down")
        def imagebuttoneventleft(self):
            self.imagebuttonevent(buttondirection="left")

        def imagebuttonevent(self, buttondirection):
            ignore_event = False
            snake = self.snake

            if snake.paused or snake.game_over or not snake.started:
                return None

            direction_change_to = snake.new_direction
            ...
            if buttondirection != None:
                ...
                direction_change_to = buttondirection
                ignore_event = True

            if opposites[snake.current_direction] != direction_change_to:
                snake.new_direction = direction_change_to
